...
print("\n" + "=" * 30)
print(f'Type of training date: {type(x_train)}')
...
X_train, X_val, X_test = x_train[x_train_filter_01], x_val[x_val_filter_01], x_test[
    x_test_filter_01]
...
data_loader = torch.utils.data.DataLoader(
    list(zip(X_train, Y_train_hot)), batch_size=64, shuffle=True, drop_last=True
)
val_data_loder = torch.utils.data.DataLoader(
    list(zip(X_val, Y_val_hot)), batch_size=64, shuffle=True, drop_last=True
)
epochs = 10
count = 0
